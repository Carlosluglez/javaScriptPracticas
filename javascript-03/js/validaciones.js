...
function esNif(cadenaNif) {
    let cadena_errores = "";
    let regExLongitud = /^.{9}/;
    let regExDni=/^[XLYKZM\d]\d{7}[A-Z]$/

    if (!regExLongitud.test(document.formulario.cif_nif.value.toUpperCase())) {
        cadena_errores = "Se ha introducido un dato no válido. No es NIF ni un DNI";
    ...
    if (!regExDni.test(document.formulario.cif_nif.value.toUpperCase())) {
        cadena_errores = "Se ha introducido un dato no válido. No es NIF ni un DNI";
        document.formulario.error_cif_nif.value=cadena_errores;
        document.formulario.error_cif_nif.style.visibility="visible";
        document.formulario.error_cif_nif.style.border="1px solid black";
